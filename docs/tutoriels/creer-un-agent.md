...
L'agent récupère toutes les actions disponibles grâce à `generateActionSchema,`il les **convertit en une liste d'actions claires, i**l **envoie cette liste au LLM** sous forme d'un **prompt structuré et** le **LLM analyse la requête utilisateur** et choisit **quelle action exécuter**.
